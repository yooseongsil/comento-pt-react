...
const renderer = new Proxy({
  ...
  set: (obj, prop, value) => {
    if (prop === 'seasonal' || prop === 'collections') {
      if (typeof value !== 'object' || value?.isArray()) return alert('Illegal value provided');
      [prop].innerHTML = value.map(createProductCard).join('');
    }
    obj[prop] = value;
  }
})

renderer.collections = products.filter((product) => product.category === 'collection');
renderer.seasonal = products.filter((product) => product.category === 'collection');
